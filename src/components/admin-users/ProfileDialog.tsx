    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-slate-900 text-white border-red-500">
        ...
        <div className="space-y-4 py-4">
          <div>
            <Label className="text-orange-200">Email (не меняется)</Label>
            <Input
              value={currentUser?.email || ""}
              disabled
              className="bg-slate-800 border-red-500 text-gray-400"
            />
          </div>
          <div>
          ...
          <div>
            <Label className="text-orange-200">Новый пароль (оставьте пустым если не меняете)</Label>
            <Input
              type="password"
